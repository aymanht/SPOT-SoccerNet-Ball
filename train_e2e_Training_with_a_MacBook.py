...
from util.device import get_device, get_grad_scaler, get_amp_context

# Réduit de 500000 à 200000 pour entraînement plus rapide sur M4 Pro
# (625 batches → 250 batches par epoch, ~4h45 → ~1h50 par epoch)
...
class E2EModel(BaseRGBModel):
    ...
    def __init__(self, num_classes, feature_arch, temporal_arch, clip_len,
                 modality, device=None, multi_gpu=False):
        # Auto-detect device: MPS (Apple M4 Pro) > CUDA > CPU
        if device is None:
    ...
    def epoch(self, loader, optimizer=None, scaler=None, lr_scheduler=None,
        ...
        with torch.no_grad() if optimizer is None else nullcontext():
            for batch_idx, batch in enumerate(tqdm(loader)):
                ...
                label = batch['label'].to(self.device, dtype=torch.float32 if self.device == 'mps' else None)
                ...
                with torch.amp.autocast(device_type=self.device, dtype=autocast_dtype):
    ...
    def predict(self, seq, use_amp=True):
        ...
        with torch.no_grad():
            # Use autocast based on device type
            autocast_dtype = torch.float16 if self.device == 'cuda' else torch.bfloat16
            autocast_ctx = torch.amp.autocast(device_type=self.device, dtype=autocast_dtype) if use_amp else nullcontext()
            with autocast_ctx:
            ...
            # Convertir en float32 avant numpy pour compatibilité avec bfloat16 (MPS)
...
def main(args):
    ...
    # Disable prefetch_factor if num_workers is 0 (required for MPS)
    num_train_workers = get_num_train_workers(args)
    dataloader_kwargs = {
        'shuffle': False,
